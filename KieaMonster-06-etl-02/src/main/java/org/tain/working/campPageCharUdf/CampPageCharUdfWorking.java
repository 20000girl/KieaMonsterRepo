package org.tain.working.campPageCharUdf;
...
@Component
@Slf4j
public class CampPageCharUdfWorking {
	...
	public void selectAllAndSend() throws Exception {
		...
		if (Flag.flag) {
			...
			info.put("url", "http://localhost:8080/v0.1/rest/campPageCharUdf/list");
			//info.put("url", "http://localhost:8080/v0.1/rest/campPageCharUdf/list2");
			info.put("method", "GET");
			...
			//
			if (Flag.flag) {
				ArrayNode arrCampPageCharUdf = recv.getArrayNode("_response");
				System.out.println(">>>>> arrCampPageCharUdf.size() = " + arrCampPageCharUdf.size());
				//JsonNode board = arrCampPageCharUdf.get(0);
				this.index = 0;
				arrCampPageCharUdf.forEach(jsonNode -> {
	...
	private void post(JsonNode jsonNode) throws Exception {
		...
		if (Flag.flag) {
			...
			// 
			MonJsonNode info = new MonJsonNode("{}");
			info.put("url", "http://localhost:8083/v0.1/api/campPageCharUdfs");
